...
const fetchIIFE = (async () => {
  ...
  getCountry.addEventListener("change", async () => {
    ...
    for (const key in apiData) {
      if (getCountry.value == apiData[key].name.common) {
        setData(apiData, key);
      }
    }
  });
...
const register = () => {
  //validations
  const inputValueObj = {
    organization: getOrganization.value,
    firstName: getFirstName.value,
    lastName: getLastName.value,
    dateOfBirth: getDob.value,
    mobile: getMobile.value,
    email: getEmail.value,
    country: getCountry.value,
    state: getState.value,
    city: getCity.value,
    communicationaddress: getCommunicationAddress.value,
    permanentaddress: getPermanentAddress.value,
    pincode: getPincode.value,
  };
  ...
  if (setImage.src.includes("images/whiteBG.png") || checkInputs()) {
    if (setImage.src.includes("images/whiteBG.png")) {
      addHelper(getImage);
    }
    for (const input in inputValueObj) {
      if (!inputValueObj[input]) addHelper(document.getElementById(`${input}`));
    }
  } else if (new Date(inputValueObj.dateOfBirth) > new Date(Date.now())) {
    addHelper(getDob);
  } else if (
    !regexMobile.test(inputValueObj.mobile) ||
    inputValueObj.mobile.replaceAll(" ", "").length > 15
  ) {
    addHelper(getMobile);
  } else if (!regexEmail.test(inputValueObj.email)) {
    addHelper(getEmail);
  } else if (
    !regexPincode.test(inputValueObj.pincode) ||
    inputValueObj.pincode.length > 6
  ) {
    addHelper(getPincode);
  } else {
    let items = {
      image: setImage.src,
      organization: inputValueObj.organization,
      firstName: inputValueObj.firstName,
      lastName: inputValueObj.lastName,
      gender,
      dateOfBirth: inputValueObj.dateOfBirth.split("-").reverse().join("-"),
      mobile: inputValueObj.mobile,
      email: inputValueObj.email,
      addressCopy,
      country: inputValueObj.country,
      state: inputValueObj.state,
      city: inputValueObj.city,
      communicationaddress: inputValueObj.communicationaddress,
      permanentaddress: inputValueObj.permanentaddress,
      pincode: inputValueObj.pincode,
    };
    createData(items);
    resetInputs();
  }
...
const createData = (data) => {
  //create object and set item to locale storage
...
const readData = () => {
  //read data and set table
  container.innerHTML = "";
  let prevData = JSON.parse(localStorage.getItem("crud") || "[]");
  if (prevData.length) {
    container.parentElement.classList.remove("display-none");
    for (const loop of prevData) {
      //looping to asssign table values
      let dataArray = [
        loop.firstName,
        loop.lastName,
        loop.organization,
      ...
      const dataElement = document.createElement("td");
      dataElement.innerHTML = `<i id='edit_${prevData.indexOf(
        loop
      )}' class="fa-solid fa-edit padding_ten blue"></i>
            <i id='delete_${prevData.indexOf(
              loop
            )}' class="fa-solid fa-trash padding_ten red"></i>`;
      rowElement.append(dataElement);
      container.append(rowElement);
      document
        .getElementById(`edit_${prevData.indexOf(loop)}`)
        .addEventListener("click", () => {
          editing = prevData.indexOf(loop);
          updateData(prevData.indexOf(loop));
        });
      document
        .getElementById(`delete_${prevData.indexOf(loop)}`)
        .addEventListener("click", () => {
          deleteData(prevData.indexOf(loop));
        });
    }
  } else {
...
const updateData = async (index) => {
  //update data and renders to table
  resetInputs();
  editing >= 0
    ? ((registerBTN.value = "UPDATE"),
      (container.style.pointerEvents = "none"),
      (getPermanentAddress.readOnly = true))
    : "REGISTER";
  ...
  for (let index = 0; index < elementArray.length; index++) {
    elementArray[index] = getDob
      ? (elementArray[index].value = `${data[valueArray[index]]}`
          .split("-")
          .reverse()
          .join("-"))
      : (elementArray[index].value = data[valueArray[index]]);
  ...
  for (const key in apiData) {
    if (elementArray[getCountry] == apiData[key].name.common) {
      setData(apiData, key);
    }
  }
...
getImage.addEventListener("change", () => {
  //event listerner for image
  let extension = getImage.files[0]
    ? getImage.files[0].name.split(".").pop().toLowerCase()
    : null;
  if (
    getImage.files[0] &&
    (extension == "jpg" || extension == "jpeg" || extension == "png") &&
    getImage.files[0].size < 1600000
  ) {
...
getCopyAddress.addEventListener("change", () => {
  //radio button for copy address
  getCopyAddress.checked
    ? ((getPermanentAddress.value = getCommunicationAddress.value),
      getCommunicationAddress.value ? removeHelper(getPermanentAddress) : null,
      getCommunicationAddress.addEventListener("input", copyaddress),
      (getPermanentAddress.readOnly = true))
    : (getCommunicationAddress.removeEventListener("input", copyaddress),
      (getPermanentAddress.readOnly = false),
...
); //removing helper texts
function addHelper(element) {
  //add helper test and border
  element.classList.add("border_red");
  element.nextElementSibling.classList.add("display_block");
}
function removeHelper(element) {
  //remove helper test and border
...
function onlyDigits(e) {
  //onlyDigits allowed
  char = e.charCode;
  return char == 43 || char == 45 || char == 46 || char == 69 || char == 101
    ? false
    : true;
}
function onlyMobile(e) {
  //onlyDigits allowed
  char = e.charCode;
  return char == 43 || char == 32 || (char >= 48 && char <= 57) ? true : false;
}
function onlyAlphabets(e) {
  //onlyAlphabets allowed
  let char = e.charCode;
  return (char >= 65 && char <= 90) || (char >= 97 && char <= 122) || char == 32
    ? true
    : false;
}
//reset
const resetBorder = () => {
  //reset all helper and borders
  RESET_INPUTS.forEach((input) => input.classList.remove("border_red"));
  RESET_INPUTS.forEach((input) =>
    input.nextElementSibling.classList.remove("display_block")
  );
