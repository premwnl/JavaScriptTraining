...
const skipIcon =
  '<i class="fa-solid fa-ban"></i><i class="fa-solid fa-ban"></i><i class="fa-solid fa-ban"></i>';
const reverseIcon =
  '<i class="fa-solid fa-rotate"></i><i class="fa-solid fa-rotate"></i><i class="fa-solid fa-rotate"></i>';
const drawTwoIcon =
  '<i class="fa-solid fa-plus">2</i><i class="fa-solid fa-plus">2</i><i class="fa-solid fa-plus">2</i>';
let [cards, colors, wildCards, mainDeck, playerCards, cpuCards, openCards] = [
  [
    "0",
    "1",
    "2",
...
for (let index = 0; index < mainDeck.length; index++) {
  //shuffling mainDeck
  let loop = Math.floor(Math.random() * mainDeck.length);
  if (loop)
    [mainDeck[index], mainDeck[loop]] = [mainDeck[loop], mainDeck[index]];
}
let deck = [...mainDeck]; //copying main deck of cards
deckCards.addEventListener("click", () =>
  playerTurn && !checkmatch(playerCards) ? drawDeckCard(playerCards, 1) : null
);
...
const drawFirstCard = () => {
  ...
  card.value !== "reverse"
    ? (openCards.unshift(card),
      ...
      setColor(card))
    : (deck.unshift(card), drawFirstCard());
};
const cardCreation = (element, card) => {
  //card creation
  if (card.color == "wild") {
    card.value == "wild"
      ? element.classList.add("wild")
      : element.classList.add("wildDrawFour");
  } else if (
    card.value == "skip" ||
    card.value == "reverse" ||
    card.value == "drawTwo"
...
const updateCards = (array, player, play) => {
  ...
  for (let index = 0; index < array.length; index++) {
    createAndUpdate(array[index], player);
  }
  switchPlay(array, player, play);
};
const createAndUpdate = (card, player) => {
  //common function for handling cards
  if (player) {
...
const switchPlay = (array, player, play) => {
  //switching turns
  let item = openCards[0];
  if (saidUNO && playerCards.length <= 0) {
    playerTurn = false;
    cpuTurn = true;
    showResult("player");
  } //checking winner
  ...
  (!play.took == play.drop || cpuTurn) && playerCards.length > 0
    ? ((playerTurn = !playerTurn), (cpuTurn = !cpuTurn))
    : null; //swapping turn

  item.color == "wild"
    ? (getColor.style.opacity = 100)
    : (getColor.style.opacity = 0); //show color while open card is wild
  play.drop && !player && item.color == "wild"
    ? (getColor.style.background = `${colors[Math.floor(Math.random() * 4)]}`)
    : null; //setting random color on wils cards

  play.drop &&
  (item.value == "reverse" ||
    item.value == "skip" ||
    item.value == "wildDrawFour" ||
    item.value == "drawTwo")
    ? player
      ? ((cpuTurn = false), (playerTurn = true))
      : !player
      ? ((cpuTurn = true), (playerTurn = false), checkTurn())
      : null
    : null; //waiting for power card turns

  play.took && player && !checkmatch(array)
    ? ((playerTurn = false), (cpuTurn = true))
    : play.took && player && array == playerCards && checkmatch(array)
    ? ((pause.style.opacity = 100),
      (playerTurn = true),
      (cpuTurn = false),
      pause.addEventListener("click", () => {
        //showing skip button on play chances
        playerTurn = false;
        cpuTurn = true;
        checkTurn();
        computerPlay();
  ...
  const unoButton = () => {
    //uno button event listerner
    let item = openCards[0];
    playerCards.length > 0
      ? ((clicked = true), (saidUNO = true))
      : ((clicked = false), (saidUNO = false));
    item.value == "reverse" ||
    item.value == "skip" ||
    item.color == "wild" ||
    item.value == "drawTwo"
      ? ((playerTurn = true), (cpuTurn = false), checkTurn())
      : ((playerTurn = false), (cpuTurn = true), computerPlay());
  ...
  if (play.drop && player && item.color == "wild" && playerCards.length > 1) {
    // show choose color
    colorModal.classList.add("showColor");
    cpuTurn = false;
    playerTurn = false;
...
const setOpenCard = (item, player, play) => {
  //setting open card and possible draw card
  ...
  const showColorModal = (e) => {
    //event listerner for color modal
    ...
    item.value == "wild"
      ...
      ? ((playerTurn = true), (cpuTurn = false), checkTurn())
      : null;
    chooseColors.forEach((val) =>
      val.removeEventListener("click", showColorModal)
    );
  };
  ...
  player && item.color == "wild" && play.drop
    ? chooseColors.forEach((val) =>
        val.addEventListener("click", showColorModal)
      )
    : null;
  item.value == "drawTwo"
    ? addCardsOnSet(player, 2)
    : item.value == "wildDrawFour"
    ? addCardsOnSet(player, 4)
    : null;
  setColor(item);
};
...
const dropCard = (card) => {
  ...
  if (playerTurn) {
    if (
      getColor.style.background == card.color ||
...
const drawDeckCard = (array, player) => {
  ...
  if (deck.length <= 1) {
    for (let index = 1; index < openCards.length; index++)
      deck.push(openCards.pop());
  }
  pause.style.opacity = 0;
...
const computerPlay = () => {
  ...
  setTimeout(() => {
    ...
    for (const index in cpuCards) {
      if (card.color == "wild") {
        if (
          getColor.style.background == cpuCards[index].color ||
          cpuCards[index].color == "wild"
        ) {
        ...
        if (
          cpuCards[index].color == card.color &&
          (cpuCards[index].value == "skip" ||
            cpuCards[index].value == "reverse" ||
            cpuCards[index].value == "drawTwo")
        ) {
          computerChoice(cpuCards[index]);
          drop = true;
          break;
      ...
      if (index == cpuCards.length - 1)
        !drop ? drawDeckCard(cpuCards, 0) : null;
      drop = false;
    }
  }, 2000);
};
...
const addCardsOnSet = (player, repeat, penalty) => {
  ...
  updateCards(player ? cpuCards : playerCards, player ? 0 : 1, {
    took: false,
    drop: false,
  });
  penalty
...
const distributeCard = (array, player) => {
  ...
  for (let index = 0; index < array.length; index++)
    setTimeout(() => {
      createAndUpdate(array[index], player);
    }, `${index == 0 ? 0 + player : index * 2 - player}00`);
};
const setColor = (item) =>
  (getColor.style.background = item.color ? item.color : "white"); //setting color for play
const checkTurn = () => {
